...
Incomplete (not maintained since 2024-06-20).
...
Add these packages to =home.packages= or =users.users.<name>.packages= or
...
with pkgs; [
  ...
  pandoc
  enchant
  aspellDicts.en # Necessary for en_US dictionary for jinx
  libvterm-neovim
  ghostscript
  math-preview

  # Use the overlay provided slotThe/emacs-lsp-booster-flake
  emacs-lsp-booster
...
Or you can build your own derivation for packages like =macism= which are not
included in Nixpkgs. Make sure Emacs can find your desired executables.
